(function(){
  ...
  const pctx = cursorPreviewCanvas.getContext('2d');
  const modalPreviewCanvas = document.getElementById('modalPreviewCanvas');
  const mpctx = modalPreviewCanvas.getContext('2d'); // dedicated overlay for popup live-previews
  ...
  const mfCtx = marginFadeCanvas.getContext('2d');
  ...
  const APP_VERSION = 'v0.180'; // bump this on every deploy — shown only in the About popup
  ...
  function generateDefaultPaletteColors() {
    const list = [];
    let cid = 1;
    // 9 grayscale swatches at the top (3 columns x 3 rows)
    const grays = [
  ...
  const stampScratchCtx = stampScratch.getContext('2d');
  ...
  const shapeScratchCtx = shapeScratch.getContext('2d');
  ...
  let vineTaperStart = false;
  let vineTaperFinish = false;
  let vineTaperOffshoots = false;
  let vineTaperLength = 50;
  let vineSizeJitter = 0;
  let vineRightAnglesOnly = false;
  ...
  let sprayPresets = [
    ...
    { id: 'builtin-spray', name: 'Default Spray', builtin: true,
      savedSettings: { sprayMode: 'paint', brushSize: 24, dabSize: 8, dabWidth: 8, dabHeight: 8, dabLockAspect: true, density: 30, falloff: 40, flow: 50, sizeJitterMin: 100, sizeJitterMax: 100, opacityJitterMin: 100, opacityJitterMax: 100, rotationMode: 'range', rotationMinAngle: 0, rotationMaxAngle: 0, dabShape: 'circle', pixelPerfect: false, opacity: 100, sprayCombineSameColor: false, sprayInterpolate: true, spraySnapToGrid: false },
    ...
    { id: 'builtin-vine-moss', name: 'Path: Castle Moss (Winding)', builtin: true,
      savedSettings: { sprayMode: 'path', brushSize: 8, vineDensity: 75, vineDecorSize: 8, vineRotationJitter: 60, vineMaxTurnPct: 40, vineOffshootDensity: 0, vineOffshootLength: 0, vineTaperStart: false, vineTaperFinish: false, vineTaperOffshoots: false, vineTaperLength: 10, vineSizeJitter: 25, vineRightAnglesOnly: false, vineEnableDecorations: true },
      settings: { sprayMode: 'path', brushSize: 8, vineDensity: 75, vineDecorSize: 8, vineRotationJitter: 60, vineMaxTurnPct: 40, vineOffshootDensity: 0, vineOffshootLength: 0, vineTaperStart: false, vineTaperFinish: false, vineTaperOffshoots: false, vineTaperLength: 10, vineSizeJitter: 25, vineRightAnglesOnly: false, vineEnableDecorations: true }
    },
    { id: 'builtin-vine-tech-pipes', name: 'Path: Tech Circuit Pipes', builtin: true,
      savedSettings: { sprayMode: 'path', brushSize: 10, vineDensity: 0, vineDecorSize: 10, vineRotationJitter: 0, vineMaxTurnPct: 100, vineOffshootDensity: 0, vineOffshootLength: 0, vineTaperStart: false, vineTaperFinish: false, vineTaperOffshoots: false, vineTaperLength: 0, vineSizeJitter: 0, vineRightAnglesOnly: true, vineEnableDecorations: false },
      settings: { sprayMode: 'path', brushSize: 10, vineDensity: 0, vineDecorSize: 10, vineRotationJitter: 0, vineMaxTurnPct: 100, vineOffshootDensity: 0, vineOffshootLength: 0, vineTaperStart: false, vineTaperFinish: false, vineTaperOffshoots: false, vineTaperLength: 0, vineSizeJitter: 0, vineRightAnglesOnly: true, vineEnableDecorations: false }
    },
    { id: 'builtin-vine-tech-grid', name: 'Path: Modular Tech Grid Branch', builtin: true,
      savedSettings: { sprayMode: 'path', brushSize: 12, vineDensity: 40, vineDecorSize: 16, vineRotationJitter: 0, vineMaxTurnPct: 100, vineOffshootDensity: 30, vineOffshootLength: 50, vineTaperStart: false, vineTaperFinish: false, vineTaperOffshoots: false, vineTaperLength: 0, vineSizeJitter: 0, vineRightAnglesOnly: true, vineEnableDecorations: true },
      settings: { sprayMode: 'path', brushSize: 12, vineDensity: 40, vineDecorSize: 16, vineRotationJitter: 0, vineMaxTurnPct: 100, vineOffshootDensity: 30, vineOffshootLength: 50, vineTaperStart: false, vineTaperFinish: false, vineTaperOffshoots: false, vineTaperLength: 0, vineSizeJitter: 0, vineRightAnglesOnly: true, vineEnableDecorations: true }
    },
    { id: 'builtin-vine-dotted', name: 'Path: Dotted Guide Line', builtin: true,
      savedSettings: { sprayMode: 'path', brushSize: 4, vineDensity: 100, vineDecorSize: 4, vineRotationJitter: 0, vineMaxTurnPct: 100, vineOffshootDensity: 0, vineOffshootLength: 0, vineTaperStart: true, vineTaperFinish: true, vineTaperOffshoots: false, vineTaperLength: 15, vineSizeJitter: 0, vineRightAnglesOnly: false, vineEnableDecorations: false },
      settings: { sprayMode: 'path', brushSize: 4, vineDensity: 100, vineDecorSize: 4, vineRotationJitter: 0, vineMaxTurnPct: 100, vineOffshootDensity: 0, vineOffshootLength: 0, vineTaperStart: true, vineTaperFinish: true, vineTaperOffshoots: false, vineTaperLength: 15, vineSizeJitter: 0, vineRightAnglesOnly: false, vineEnableDecorations: false }
    }
  ];
  ...
  function duplicateFrame(){
    ...
    const copiedLayers = src.layers.map(l=>{
      ...
      if(l.heightCanvas){
        ...
        hc.getContext('2d').drawImage(l.heightCanvas, 0, 0);
        newLayer.heightCanvas = hc;
        newLayer.heightCtx = hc.getContext('2d');
      }
      if(l.roughnessCanvas){
        const rc = document.createElement('canvas');
        rc.width = W; rc.height = H;
        rc.getContext('2d').drawImage(l.roughnessCanvas, 0, 0);
        newLayer.roughnessCanvas = rc;
        newLayer.roughnessCtx = rc.getContext('2d');
      }
      return newLayer;
    });
    const insertAt = currentFrameIndex + 1;
  ...
  function compositeTintedFrame(frame, tintHex, distance){
    ...
    const alpha = Math.max(0.08, 0.35 - (distance-1)*0.12);
  ...
  function hasLayerContent(layer){
    ...
    const ctx = layer.colorCtx || layer.ctx || c.getContext('2d');
    try {
  ...
  function refreshMatrixTimeline(){
    ...
    displayFrames.forEach((f, fColIdx) => {
      ...
      th.textContent = f.name || ('Frame ' + (fIdx + 1));
    ...
    for (let visualRow = 0; visualRow < currentLayerList.length; visualRow++) {
      ...
      visCheck.addEventListener('change', (ev) => {
        ev.stopPropagation();
        pushHistory();
        layerObj.visible = ev.target.checked;
        refreshLayerPanel();
        refreshMatrixTimeline();
        render();
      });
  ...
  function renderFrameForPlayback(frame){
    ...
    frame.layers.forEach(l=>{
      if(!l.visible) return;
      dctx.globalAlpha = l.opacity/100;
  ...
  function ensureHeightCanvas(layer){
    ...
    layer.heightCtx = hc.getContext('2d');
      ...
      frames.forEach((f, fIdx) => {
        ...
        if(fIdx === currentFrameIndex){
          ...
          if(f.layers[srcIdx].heightCanvas){
            const hc = document.createElement('canvas');
            hc.width = W; hc.height = H;
            hc.getContext('2d').drawImage(f.layers[srcIdx].heightCanvas, 0, 0);
            l.heightCanvas = hc;
            l.heightCtx = hc.getContext('2d');
          }
          if(f.layers[srcIdx].roughnessCanvas){
            const rc = document.createElement('canvas');
            rc.width = W; rc.height = H;
            rc.getContext('2d').drawImage(f.layers[srcIdx].roughnessCanvas, 0, 0);
            l.roughnessCanvas = rc;
            l.roughnessCtx = rc.getContext('2d');
          }
        }
        f.layers.push(l);
      });
  ...
  function updateCanvasPadding(){
    ...
    const canvasW = W * zoom + ringPx*2;
    const canvasH = H * zoom + ringPx*2;
  ...
  function applyZoom(){
    const wPx = (W * zoom) + 'px', hPx = (H * zoom) + 'px';
    ...
    cursorPreviewCanvas.style.width = wPx; cursorPreviewCanvas.style.height = hPx;
    ...
    marginFadeCanvas.style.width = ((W + MARGIN_PX*2) * zoom) + 'px';
    marginFadeCanvas.style.height = ((H + MARGIN_PX*2) * zoom) + 'px';
  }
  function setZoom(z){
  ...
  function zoomAtPoint(newZoomVal, clientX, clientY){
    ...
    const scaleX = W / oldRect.width;
    const scaleY = H / oldRect.height;
    ...
    const newScreenX = newRect.left + (imgX / W) * newRect.width;
    const newScreenY = newRect.top + (imgY / H) * newRect.height;
  ...
  function render(){
    ...
    dctx.clearRect(0,0,W,H);
    if(onionSkinEnabled){
      onionSkinCache.prev.forEach(g=>{ dctx.globalAlpha = g.alpha; dctx.drawImage(g.canvas, 0, 0); });
      onionSkinCache.next.forEach(g=>{ dctx.globalAlpha = g.alpha; dctx.drawImage(g.canvas, 0, 0); });
      dctx.globalAlpha = 1;
    }
    layers.forEach((l,idx)=>{
      ...
      if(idx === activeLayer && sprayBuffer){
        ...
        dctx.globalAlpha = (l.opacity/100) * (opacity/100);
        dctx.drawImage(sprayBuffer,0,0);
      }
      if(idx === activeLayer && floatingSelection){
        dctx.globalAlpha = l.opacity/100;
        dctx.drawImage(floatingSelection.canvas, floatingSelection.x, floatingSelection.y);
      }
      if(idx === activeLayer && pixiStrokeActive){
        ...
        pixiApp.render();
        dctx.globalAlpha = (l.opacity/100) * (opacity/100);
        dctx.imageSmoothingEnabled = !pixelPerfect;
        dctx.drawImage(pixiApp.view, MARGIN_PX, MARGIN_PX, W, H, 0, 0, W, H);
      }
    });
    dctx.globalAlpha = 1;
  ...
  function updateSeamlessPreview(){
    if(!seamlessModeEnabled) return;
    const canvas = document.getElementById('seamlessPreviewCanvas');
    if(canvas.width !== W*3 || canvas.height !== H*3){
    ...
    for(let ty=0; ty<3; ty++){
      for(let tx=0; tx<3; tx++){
        sctx.drawImage(displayCanvas, tx*W, ty*H);
      }
    }
  }
  document.getElementById('seamlessModeEnabled').addEventListener('change', e=>{
    seamlessModeEnabled = e.target.checked;
    document.getElementById('seamlessPreviewWrap').style.display = seamlessModeEnabled ? 'block' : 'none';
    updateSeamlessPreview();
  });
  ...
  let lightboxCtx = lightboxCanvas.getContext('2d');
  ...
  function computeLitPixels(){
    ...
    ensureHeightCanvas(layer);
    ensureRoughnessCanvas(layer);

    const heightData = layer.heightCtx.getImageData(0, 0, W, H).data;
    const roughnessData = layer.roughnessCtx.getImageData(0, 0, W, H).data;
    const colorData = dctx.getImageData(0, 0, W, H).data;
    const out = new Uint8ClampedArray(colorData.length);

    const lightHeightPct = +document.getElementById('lightHeightSlider').value;
    const ambient = (+document.getElementById('lightAmbientSlider').value) / 100;
    ...
    if(!lightboxImageData){
      const presetSelect = document.getElementById('lightboxPresetSelect');
      renderLightboxPreset(presetSelect ? presetSelect.value : 'studio');
    }
  ...
  function updateColorHighlight(){
    ...
    const targetKeys = new Set([...selectedColors].map(hex=>{
    ...
    for(let i=0; i<src.length; i+=4){
      ...
      if(targetKeys.has(src[i]*65536 + src[i+1]*256 + src[i+2])){
  ...
  function updateLightingPreview(){
    if(!lightingPreviewEnabled) return;
    const litData = computeLitPixels();
    if(!litData) return;
    const canvas = document.getElementById('lightingPreviewCanvas');
    if(canvas.width !== W || canvas.height !== H){ canvas.width = W; canvas.height = H; }
    const lctx = canvas.getContext('2d');
    const imgData = lctx.createImageData(W, H);
    imgData.data.set(litData);
    lctx.putImageData(imgData, 0, 0);
    // draw a small marker showing the current light position
    lctx.save();
    lctx.globalAlpha = 0.8;
    lctx.fillStyle = '#ffe27a';
    lctx.beginPath();
    lctx.arc(lightPosX*W, lightPosY*H, Math.max(2, Math.min(W,H)*0.03), 0, Math.PI*2);
    lctx.fill();
    lctx.restore();
  }
  function lightingPointerToNorm(e){
    const canvas = document.getElementById('lightingPreviewCanvas');
    const rect = canvas.getBoundingClientRect();
    const nx = (e.clientX - rect.left) / rect.width;
    const ny = (e.clientY - rect.top) / rect.height;
    return { x: Math.max(-1, Math.min(2, nx)), y: Math.max(-1, Math.min(2, ny)) };
  }
  document.getElementById('lightingPreviewEnabled').addEventListener('change', e=>{
    lightingPreviewEnabled = e.target.checked;
    document.getElementById('lightingPreviewWrap').style.display = lightingPreviewEnabled ? 'block' : 'none';
    updateLightingPreview();
  });
  ...
  document.getElementById('lightingPreviewCanvas').addEventListener('pointerdown', e=>{
    lightingDragging = true;
    const p = lightingPointerToNorm(e);
    lightPosX = p.x; lightPosY = p.y;
    updateLightingPreview();
  });
  ...
  window.addEventListener('pointerup', ()=>{ lightingDragging = false; });
  ...
  function computeSeamlessResult(fadePct, zoomPct){
    ...
    let blurredData = originalData;
    for(let i=0; i<6; i++) blurredData = boxBlurArray(blurredData, w, h);

    const bandWidth = (zoomPct/100) * 0.5; // zoom widens how far from the seam the fade reaches
    ...
    for(let y=0; y<h; y++){
      for(let x=0; x<w; x++){
        ...
        const proxX = Math.max(0, 1 - dx/bandWidth);
        const proxY = Math.max(0, 1 - dy/bandWidth);
        const proximity = Math.min(1, Math.max(proxX, proxY));
        const weight = proximity * strength;
        const i = (y*w+x)*4;
        outData[i]   = Math.round(originalData[i]  *(1-weight) + blurredData[i]  *weight);
        outData[i+1] = Math.round(originalData[i+1]*(1-weight) + blurredData[i+1]*weight);
        outData[i+2] = Math.round(originalData[i+2]*(1-weight) + blurredData[i+2]*weight);
        outData[i+3] = Math.round(originalData[i+3]*(1-weight) + blurredData[i+3]*weight);
      }
    }
  ...
  function drawGridOverlay(){
    ...
    grids.forEach(g=>{
      ...
      for(let x = startX; x <= W; x += g.spacing){
        gctx.moveTo(Math.floor(x)+0.5, 0);
        gctx.lineTo(Math.floor(x)+0.5, H);
      }
      for(let y = startY; y <= H; y += g.spacing){
        gctx.moveTo(0, Math.floor(y)+0.5);
        gctx.lineTo(W, Math.floor(y)+0.5);
      }
      gctx.stroke();
      gctx.restore();
    });
  ...
  async function pasteFromClipboard(){
    ...
    lifted.getContext('2d').drawImage(pasteSourceImg, 0, 0);
  ...
  function copySelectionToClipboard(){
    ...
    internalClipboardCanvas.getContext('2d').drawImage(selCanvas, 0, 0);
  ...
  document.getElementById('stampFromSelectionBtn').addEventListener('click', ()=>{
    ...
    try {
      const userEntered = window.prompt('Stamp name:', defaultName);
  ...
  const MAX_TINTED_CACHE_SIZE = 500;
  ...
  function getRotatedRectBounds(cx, cy, w, h, angle, pivotX = 0.5, pivotY = 0.5) {
    ...
    return {
      x: Math.floor(minX) - 1,
      y: Math.floor(minY) - 1,
      w: Math.ceil(maxX) - Math.floor(minX) + 2,
      h: Math.ceil(maxY) - Math.floor(minY) + 2
    };
  }
  ...
  function drawBuiltStamp(ctx, sourceCanvas, w, h, cx, cy, angle, pivotX = 0.5, pivotY = 0.5){
    // Places the already-built tinted stamp using nearest-neighbor or bilinear based on pixelPerfect.
    ctx.save();
    ...
    ctx.drawImage(sourceCanvas, 0, 0, w, h, destX, destY, w, h);
    ctx.restore();
  }
  function drawStampDab(ctx, maskCanvas, cx, cy, targetW, targetH, color, angle, pivotX = 0.5, pivotY = 0.5){
    ...
    if(built){
      drawBuiltStamp(ctx, built.canvas, built.w, built.h, cx, cy, finalAngle, finalPivotX, finalPivotY);
    }
  }
  ...
  const shpctx = stampHoverCanvas.getContext('2d');
  ...
  function refreshStampList(){
    ...
    if(!stamps || stamps.length === 0){
      ...
      const vss = document.getElementById('vineStampSourceSelect'); if(vss) vss.value = selectedStampIndex;
  ...
  document.getElementById('stampSelect')?.addEventListener('change', e=>{
    ...
    const vss = document.getElementById('vineStampSourceSelect'); if(vss) vss.value = idx ?? '';
    refreshStampList();
  ...
  function populateStampDropdown(){
    ...
    if(vineSel){
      ...
      noneOpt.value = ''; noneOpt.textContent = 'No stamp (use leaf)';
      vineSel.appendChild(noneOpt);
      stamps.forEach((s, idx)=>{
        const opt = document.createElement('option');
        opt.value = idx; opt.textContent = s.name;
    ...
    if(vineSel) vineSel.value = selectedStampIndex ?? '';
  ...
  document.getElementById('stampSourceSelect').addEventListener('change', e=>{
    ...
    if(vss) vss.value = selectedStampIndex ?? '';
    refreshStampList();
    drawStampPivotCanvas();
  });
  ...
  function canvasCoords(e){
    ...
    if (isPathTool() && (vineRightAnglesOnly || brushPixelPerfect)) {
      x = Math.round(x);
      y = Math.round(y);
    }
    return { x, y };
  ...
  function brushStampAt(px, py){
    ...
    if(!built) return;
    const bounds = getRotatedRectBounds(px, py, built.w, built.h, 0, pX, pY);
    paintNoBlend(ctx, W, H, bounds.x, bounds.y, bounds.w, bounds.h, (sctx)=>{
      sctx.globalAlpha = opacity/100;
      drawBuiltStamp(sctx, built.canvas, built.w, built.h, px, py, 0, pX, pY);
    });
    const nearEdge = px < brushSize || px > W-brushSize || py < brushSize || py > H-brushSize;
    if(nearEdge){
      marginHasContent = true;
      paintNoBlend(mfCtx, W+MARGIN_PX*2, H+MARGIN_PX*2, bounds.x+MARGIN_PX, bounds.y+MARGIN_PX, bounds.w, bounds.h, (sctx)=>{
        sctx.globalAlpha = opacity/100;
        drawBuiltStamp(sctx, built.canvas, built.w, built.h, px+MARGIN_PX, py+MARGIN_PX, 0, pX, pY);
      });
    }
  }
  ...
  function brushCircleAt(px, py){
    ...
    const ctx = layers[activeLayer].ctx;
    const r = brushSize/2;
    paintNoBlend(ctx, W, H, px-r, py-r, r*2, r*2, (sctx)=>{
      sctx.globalAlpha = opacity/100;
    ...
    if(nearEdge){
      marginHasContent = true;
      paintNoBlend(mfCtx, W+MARGIN_PX*2, H+MARGIN_PX*2, px+MARGIN_PX-r, py+MARGIN_PX-r, r*2, r*2, (sctx)=>{
        sctx.globalAlpha = opacity/100;
        sctx.fillStyle = fgColor;
        sctx.beginPath();
        sctx.arc(px+MARGIN_PX, py+MARGIN_PX, r, 0, Math.PI*2);
        sctx.fill();
      });
    }
  }
  ...
  // ---------- No-blend compositing ----------
  // Paint operations replace pixels outright rather than alpha-blending with what's already
  // there — a semi-transparent stroke can exist, but repainting over it (even with more
  // semi-transparent paint) replaces it, never compounds. Canvas 2D's native compositing modes
  // either still blend (source-over) or wipe the ENTIRE canvas (copy), not just the drawn
  // shape's footprint, so this works by drawing the shape normally onto a small transparent
  // scratch canvas first, then directly overwriting (not compositing) only the pixels the
  // scratch actually covers onto the real layer. Works uniformly for any shape — square, circle,
  // or an arbitrary stamp alpha mask — since it only cares about the scratch's final pixel
  // result, not how it was drawn.
